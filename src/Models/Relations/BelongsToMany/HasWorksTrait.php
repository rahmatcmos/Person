...
trait HasWorksTrait {
	...
	{
		return $query->whereHas('works', function($q)use($variable){$q->whereNull('end')->orderBy('start', 'asc');})->with(['works.branch.organisation' => function($q)use($variable){$q->where('organisations.name', 'like', '%'.$variable.'%');}]);
	}
